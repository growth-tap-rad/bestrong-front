...
let currentWeight = 0;
let currentName = '';
const weightHistoric = ref([])
const inputWeights = ref([])
...
const getUserProfile = async () => {
  ...
  profileStore.getUser(appStore.getCurrentQueryDate).then(data => {

    const progresses = data.progress

    weightHistoric.value = progresses.map(progress => parseFloat(progress.weight))

    inputWeights.value = progresses.map(progress => {
  ...
  <div class="profile">
    <header class="header">
      ...
      <button disabled class="buttonIcon">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#e50000" class="bi bi-gear-fill"
          viewBox="0 0 16 16">
          <path
    ...
    <main class="main">
      ...
      <section v-if="!isEditing && weightHistoric.length > 1" class="Progress">
